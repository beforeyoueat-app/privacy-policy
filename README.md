# Privacy Policy for Before You Eat

*Last updated: May 17, 2026*

## Information We Collect
We collect your name and email address when you sign in with Google. We store meal and nutrition data you enter in the app on our servers.

## Camera
The app can access your camera to photograph meals for AI nutrition analysis. Images are sent to OpenAI for processing and are not stored permanently.

## Third Party Services
- Google Sign-In (authentication)
- OpenAI (AI nutrition analysis)
- Google AdMob (advertising)
- Google Play Billing (subscriptions)

## GDPR
If you are located in the EU, you have the right to access, correct, or delete your personal data. Contact us at beforeyoueat@proton.me to exercise these rights.

## Children
This app is not directed at children under 13.

## Contact
beforeyoueat@proton.me
